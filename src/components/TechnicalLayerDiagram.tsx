    ...
    return (
      <div className={`bg-slate-50 border border-slate-200 rounded-xl p-4 ${className}`}>
        <h4 className="text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2 flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-[#0a1680]"></span>
          {isEn ? 'Molding & Transition Profiles' : 'Perfiles de Moldura & Ensamble'}
        </h4>
        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          <div className="p-2 bg-white rounded border border-slate-200">
            ...
            <div className="text-[9px] text-[#0a1680] mt-1 font-semibold">
  ...
  if (type === 'wpc-slat') {
    return (
      <div className={`bg-slate-50 border border-slate-200 rounded-xl p-4 ${className}`}>
        <h4 className="text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2 flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-[#0a1680]"></span>
          {isEn ? 'WPC Slat Acoustic Structure' : 'Estructura Ranurada WPC / Acoustic Slat'}
        </h4>
        <div className="flex items-center gap-3">
          <div className="flex-1 bg-white p-3 rounded-lg border border-slate-200">
            <div className="flex items-center gap-1 h-10">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="flex-1 h-full bg-[#0a1680]/20 border-t-2 border-r border-[#0a1680] rounded-t flex items-center justify-center">
            ...
            <div className="flex justify-between text-[10px] text-slate-500 mt-2">
              <span>{isEn ? 'Slotted fluted slat' : 'Listón ranurado'}</span>
              <span>{isEn ? 'Acoustic base / WPC core' : 'Base acústica / Núcleo WPC'}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`bg-slate-50 border border-slate-200 rounded-xl p-4 ${className}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[#0a1680]/10 text-[#0a1680]">
      ...
      <div className="space-y-1.5 text-xs">
        ...
        {/* Layer 2: Wear Layer */}
        <div className="flex items-center gap-3 bg-white p-2 rounded border border-slate-200">
          <span className="w-6 h-3 bg-blue-300 border border-blue-400 rounded shrink-0"></span>
          <div className="flex-1 flex justify-between">
            <span className="font-medium text-slate-700">
              {isEn
            ...
            <span className="text-[11px] text-[#0a1680] font-bold">
